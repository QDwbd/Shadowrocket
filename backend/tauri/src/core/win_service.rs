...
use crate::{
    config::{shadowrocket::ClashCore, Config},
    utils::dirs,
};
...
/// Install the Shadowrocket Service
...
/// Uninstall the Shadowrocket Service
/// 该函数应该在协程或者线程中执行，避免UAC弹窗阻塞主线程
...
pub async fn check_service() -> Result<JsonResponse> {
    ...
    let response = reqwest::ClientBuilder::new()
        ...
        .send()
        .await
        .context("failed to connect to the Shadowrocket Service")?
        .json::<JsonResponse>()
        .await
        .context("failed to parse the Shadowrocket Service response")?;
