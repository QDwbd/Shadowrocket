...
async function resolve() {
  ...
  const appPathList = [
    join("macos/Shadowrocket.aarch64.app.tar.gz"),
    join("macos/Shadowrocket.aarch64.app.tar.gz.sig"),
  ];
  ...
  fs.copyFileSync(join("macos/Shadowrocket.app.tar.gz"), appPathList[0]);
  fs.copyFileSync(join("macos/Shadowrocket.app.tar.gz.sig"), appPathList[1]);
  ...
  await uploadAssets(release.id, [
    join(`dmg/Shadowrocket_${version}_aarch64.dmg`),
    ...appPathList,
  ]);
}
